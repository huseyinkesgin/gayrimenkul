<flux:modal :name="'sube-duzenleme-modal'" class="md:w-[600px] max-w-2xl" x-on:close="$wire.resetModal()">
    <div class="space-y-6">
        <div>
            <flux:heading size="lg">Şube Düzenle</flux:heading>
            <flux:text class="mt-2">Şube bilgilerini düzenleyebilirsiniz.</flux:text>
        </div>
        ...
        @elseif($sube)
            <form wire:submit.prevent="updateSube" class="space-y-4">
                <!-- Şube Bilgileri -->
        ...
        @elseif($subeId)
            <div class="text-center py-8">
                <flux:text>Şube bulunamadı.</flux:text>
            </div>
        @else
            <div class="text-center py-8">
                <flux:text>Düzenlenecek şube seçiniz.</flux:text>
            </div>
        @endif
    </div>
</flux:modal>
