<flux:modal :name="'pozisyon-duzenleme-modal'" class="md:w-[500px] max-w-lg" x-on:close="$wire.resetModal()">
    <div class="space-y-6">
        <div>
            <flux:heading size="lg">Pozisyon Düzenle</flux:heading>
            <flux:text class="mt-2">Pozisyon bilgilerini düzenleyebilirsiniz.</flux:text>
        </div>
        ...
        @elseif($pozisyon)
            <form wire:submit.prevent="updatePozisyon" class="space-y-4">
        ...
        @elseif($pozisyonId)
            <div class="text-center py-8">
                <flux:text>Pozisyon bulunamadı.</flux:text>
            </div>
        @else
            <div class="text-center py-8">
                <flux:text>Düzenlenecek pozisyon seçiniz.</flux:text>
            </div>
        @endif
    </div>
</flux:modal>
