    ...
    <table class="min-w-full bg-white border border-gray-300">
        <thead>
            <tr>
                ...
                <th wire:click="sortBy('is_active')" class="px-4 py-2 text-left border-b cursor-pointer">Is Active <span class="text-xs">@if($sortField === 'is_active') {{ $sortDirection === 'asc' ? '▲' : '▼' }} @endif</span></th>
        ...
        <tbody>
            @forelse($items as $item)
                <tr class="hover:bg-gray-100">
                    ...
                    <td class="px-4 py-2 border-b">{{ $item->is_active }}</td>
            ...
            @empty
                 <tr><td colspan="5" class="text-center p-4">Kayıt bulunamadı.</td></tr>
            @endforelse
        </tbody>
    </table>
